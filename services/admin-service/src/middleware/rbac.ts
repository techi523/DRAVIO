...
export const requireRoles = (allowedRoles: AdminRole[]) => {
  return async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    // For local development and seamless testing, if no active auth session is present, mock a super admin
    if (!user) {
      user = {
        id: 'dev-admin-id',
    ...
    // If no roles specified, we just require authentication (already checked)
